    ...
    this.rows = Object.values(room.rounds).map(round => {
      const estimates = Object.entries(round.estimates)
        .reduce((acc, [id, estimate]) => {
          acc[id] = this.converter.transform(estimate, room.cardSet, 'exact').toString();
      ...
      const average = estimateValues.reduce((acc, curr) => acc + curr, 0) / estimateValues.length;

      const result = {
        estimates,
        average: this.converter.transform(average, room.cardSet, 'rounded').toString(),
...
export class SerializerService {
  ...
  exportRoomAsCsv(room: Room) {
    try {
      ...
      link.setAttribute("download", 'Planning Poker Session - ' + new Date().toLocaleString());
