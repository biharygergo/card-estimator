...
export function getArticles(): Article[] {
  return (
    readdirSync(path.join(__dirname, 'data'))
      // readdirSync order is filesystem-dependent - alphabetical on macOS, but
      // arbitrary on the ext4 volumes CI runs on. Sort so index.json and
      // routes.txt are byte-identical wherever they are generated, which is
      // what lets the PR workflow diff them against the committed copies.
      .sort()
      .map(fileName => {
